...
local function Transform(source, zone)
	...
	if PlayersTransforming[source] == true then
		...
		if zone == "WeedProcess" then
			...
			if itemQuantity <= 2 or itemBag <= 0 then
				TriggerClientEvent('esx:showNotification', source, _U('no_necesario'))
				return
			else
		...
		if zone == 'WeedSell' then
			...
			if weed == 0 then
				...
				SetTimeout(1100, function()
					local money = math.random(2500,2500)
					xPlayer.removeInventoryItem('weed_pooch', 1)
					xPlayer.addAccountMoney('black_money', money)
					TriggerClientEvent('esx:showNotification', xPlayer.source, 'Recibiste '..money..'$')
					Sell(source,zone)
				end)
			end
